...
Expected pattern: /ng-invalid|field-info warning|exceeded/i
Timeout: 5000ms
...
```ts
  ...
  61  |       // Після набору максимуму сайт додає клас на лічильник або показує 171/170
  62  |       //await expect(createNewsPage.titleCounter).toContainText('171/170');
> 63  |       await expect(createNewsPage.titleCounter).toHaveClass(/ng-invalid|field-info warning|exceeded/i);
      |                                                 ^ Error: expect(locator).toHaveClass(expected) failed
  64  |     });
  65  |   });
  66  | 
